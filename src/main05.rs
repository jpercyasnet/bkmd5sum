                   ...
                   if mountv != "--" {
                       ...
                       if !self.offline_bool {
                           if let Err(e) = connectdb(&self.dbconn) {
                               ...
                               self.msg_value = errstrd.to_string();
                               if errcoded == 0 {
                                   match self.dbconn.prepare("SELECT  rowid, refname, filename, dirname, filesize, filedate, md5sum
                                      FROM blubackup
                                      WHERE refname = :fil
                                      LIMIT 1") {
                                         Ok(mut stmt) => {
...
//               let mut inputstr: String = self.targetdir.clone();
    ...
    fn view(&self) -> Element<Message> {
        column![
            ...
            row![checkbox("Offline (no database)", self.offline_bool, Message::Offline,).size(20),
                 button("get database").on_press(Message::DBPressed).style(theme::Button::Secondary),
            ...
            row![checkbox("Alternative label", self.alt_bool, Message::Alt,).size(20),
                 button("check if in db").on_press(Message::CheckDBPressed).style(theme::Button::Secondary),
...
//    const TOTAL: u16 = 807;
    ...
    async fn execit(bkpath: String, targetdir: String, labelname: String,  targetname: String, tx_send: mpsc::UnboundedSender<String>,) -> Result<Execx, Error> {
     let mut errstring  = "Complete blu ray listing".to_string();
...
//                  let (errcod, errstr, md5sumv) = findmd5sum(fullpath.clone());
                  ...
                  let msgx = format!("Progress|{}|{}| elapsed time {:.1} mins at {}", szaccum, totalsz, minsx, datexx.format("%H:%M:%S"));
                  tx_send.unbounded_send(msgx).unwrap();
              }
          }
        }
